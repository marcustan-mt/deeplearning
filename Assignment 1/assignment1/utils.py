...
def load_mnist_trainval():
    ...
    #############################################################################
    train_amt = int(0.8 * len(data))
    
    train_data = data[:train_amt]
    train_label = label[:train_amt]

    val_data = data[train_amt:]
    val_label = label[train_amt:]
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
...
def generate_batched_data(data, label, batch_size=32, shuffle=False, seed=None):
    ...
    if shuffle == True:
        shuffled_data_label = list(zip(data, label))
        random.shuffle(shuffled_data_label)
        data, label = zip(*shuffled_data_label)
...
def plot_curves(train_loss_history, train_acc_history, valid_loss_history, valid_acc_history, extra_label):
    """
    ...
    #############################################################################
    plt.plot(range(len(train_loss_history)),train_loss_history, label=f'Train Loss History')
    plt.plot(range(len(valid_loss_history)), valid_loss_history, label = f'Validation Loss History')
    plt.title(f'Loss Curves, {extra_label}')
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    ...
    plt.plot(range(len(train_acc_history)),train_acc_history, label=f'Train Acc History')
    plt.plot(range(len(valid_acc_history)), valid_acc_history, label = f'Validation Acc History')
    plt.title(f'Acc Curves, {extra_label}')
    plt.xlabel('Epochs')
    plt.ylabel('Accuracy')
    plt.legend(loc="upper left")
